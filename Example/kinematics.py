...
from Example.util import RotMatrix3D, point_to_rad


class kinematics:
    def __init__(self, right_front_id, right_back_id, left_front_id, left_back_id):
        left_front_leg = left_front_id
        left_back_leg = left_back_id
        right_front_leg = right_front_id
        right_back_leg = right_back_id
        ...
        self.right_legs = [right_front_leg, right_back_leg]
    ...
    def leg_IK(
        self, xyz, rot=[0, 0, 0], legID=0, is_radians=True, center_offset=[0, 0, 0]
    ):
        ...
        XYZ = asarray(
            (
                inv(RotMatrix3D(rot, is_radians))
                * (
                    (
                        array(xyz) + self.leg_origins[legID, :] - array(center_offset)
                    ).transpose()
                )
            ).transpose()
        )
        ...
        xyz_ = asarray(
            XYZ - self.leg_origins[legID, :] + array(center_offset)
        ).flatten()
    ...
    def leg_IK_calc(self, xyz, is_right=False):
        ...
        b_2 = acos(
            (self.link_2**2 + len_B**2 - self.link_3**2)
            / (2 * self.link_2 * len_B)
        )
        b_3 = acos(
            (self.link_2**2 + self.link_3**2 - len_B**2)
            / (2 * self.link_2 * self.link_3)
        )
        ...
        # calculate joint 3
        j3_ = np.reshape(
            np.array([self.link_2 * cos(theta_2), 0, self.link_2 * sin(theta_2)]),
            [3, 1],
        )
        ...
        angles = self.angle_corrector(
            angles=[theta_1, theta_2, theta_3], is_right=is_right
        )
    ...
    def base_pose(self, rot=[0, 0, 0], is_radians=True, center_offset=[0, 0, 0]):
        ...
        offset = (
            RotMatrix3D(rot, is_radians) * (matrix(center_offset).transpose())
            - matrix(center_offset).transpose()
        )
        ...
        rotated_base = (
            RotMatrix3D(rot, is_radians) * self.leg_origins.transpose() - offset
        )
    ...
    # plot leg
    def plot_leg(
        self, ax, xyz, rot=[0, 0, 0], legID=0, is_radians=True, center_offset=[0, 0, 0]
    ):
        # get coordinates
        p = (
            self.leg_pose(xyz, rot, legID, is_radians, center_offset)
            + self.base_pose(rot, is_radians, center_offset)[legID]
        ).transpose()
        # plot coordinates
        ax.plot3D(
            asarray(p[0, :]).flatten(),
